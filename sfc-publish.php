...
<?php
	...
	if ($options['fanpage'] && $options['page_access_token']) {
	...
	} else if ($options['fanpage'] && !$options['page_access_token']) {
...
function sfc_publish_meta_box( $post ) {
	...
	if (!empty($post->post_excerpt)) $attachment['description'] = sfc_publish_make_excerpt($post->post_excerpt);
	else $attachment['description'] = sfc_publish_make_excerpt($post->post_content);
	...
	<?php
	if ($options['fanpage']) $ui['actor_id'] = $options['fanpage'];
	else $ui['actor_id'] = $options['appid'];
	?>
...
function sfc_publish_automatic($id, $post) {
	...
	if (!empty($image)) $attachment['picture'] = $image;
