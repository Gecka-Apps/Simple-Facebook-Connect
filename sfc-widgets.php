...
function get_sfc_fanbox($args='') {
	...
	$retvar = '<fb:like-box profile_id="'.$id.'" logobar="'.$logobar.'" stream="'.$stream.'" connections="'.$connections.'" width="'.$width.'" colorscheme="'.$colorscheme.'"';
...
class SFC_User_Status_Widget extends WP_Widget {
	...
	function widget($args, $instance) {
		...
		if ( false === ($statuses = get_transient($this->get_field_id('statuses')))) {
...
function sfc_chicklet($id = 0) {
	...
	if (!isset($sfc_chicklet_fancount[$id])) {
		...
		$data = wp_remote_get(urlencode("https://api.facebook.com/method/fql.query?format=json&query=select page_url, fan_count from page where page_id = {$id}"));
		if (!is_wp_error($data)) {
			$resp = json_decode($data['body'],true);
			if (!empty($resp)) {
				$sfc_chicklet_fancount[$id] = $resp;
